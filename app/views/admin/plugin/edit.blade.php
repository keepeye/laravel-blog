...
<div class="row">
    ...
    {{ Form::open(array('route'=>['admin.article.update',$article->id],'method'=>'put','role'=>'form')) }}
    <div class="col-md-9">
        <div class="panel panel-default">
            <div class="panel-body">
                <div class="form-group">
                    <label for="title">标题</label>
                    <input class="form-control" id="title" name="data[title]" value="{{$article->title}}" type="text" placeholder="输入标题.." required/>
                </div>

                <div class="form-group">
                    <label for="description">摘要</label>
                    <span class="help-block">写一段关于文章的简要介绍，将显示在列表页</span>
                    <textarea class="form-control" id="description" name="data[description]"  rows="3" placeholder="输入摘要..">{{$article->description}}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="article_content" name="data[content_md]"  rows="10">{{$article->content_md}}</textarea>
                </div>
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <div class="form-group">
                    ...
                    <input class="form-control" name="data[created_at]" id="created_at" value="{{$article->created_at}}" type="text"/>
                </div>
                <div class="form-group">
                    <label>分类</label>
                    <select class="form-control" name="data[cid]">
                        ...
                        <option value="{{$category['id']}}" @if($category['final'] == 0) disabled @endif @if($article->cid == $category['id']) selected @endif  >@if($category['level']>0){{str_repeat('&nbsp;',$category['level']*5).'∟'}}@endif{{$category['name']}}</option>
